...
@register("proactive_chat", "L1uStark", "主动发言插件，支持沉默触发、自我学习、连续发言限制", "1.4.0")
class ProactiveChatPlugin(Star):
    ...
    async def on_message(self, event: AstrMessageEvent):
        ...
        if not isinstance(raw_message, list) or len(raw_message) == 0:
            logger.info(f"[日志] {chat_type} {chat_id} 空消息或输入状态，忽略")
            return
        ...
        logger.info(f"[日志] {chat_type} {chat_id} 消息段类型: {seg_types}")

        if not has_content:
            logger.info(f"[日志] {chat_type} {chat_id} 无有效内容（输入状态等），忽略")
            return
